# gx-grid-smart-cell

<!-- Auto Generated Below -->

## Properties

| Property             | Attribute              | Description                                                                                                                                                                | Type      | Default     |
| -------------------- | ---------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- | ----------- |
| `cssClass`           | `css-class`            | A CSS class to set as the `gx-grid-smart-cell` element class.                                                                                                              | `string`  | `undefined` |
| `highlightable`      | `highlightable`        | True to highlight control when an action is fired.                                                                                                                         | `boolean` | `true`      |
| `isRowEven`          | `is-row-even`          | Whether this row is even position or not. This is specially required in Virtual scroll scenarios where the position in the DOM is not the real position in the collection. | `boolean` | `false`     |
| `rowsLandscape`      | `rows-landscape`       | Specifies the number of rows that will be displayed in the landscape mode.                                                                                                 | `number`  | `null`      |
| `rowsPortrait`       | `rows-portrait`        | Specifies the number of rows that will be displayed in the portrait mode.                                                                                                  | `number`  | `null`      |
| `showHorizontalLine` | `show-horizontal-line` | True to show horizontal line.                                                                                                                                              | `boolean` | `false`     |

---

_Built with [StencilJS](https://stenciljs.com/)_
